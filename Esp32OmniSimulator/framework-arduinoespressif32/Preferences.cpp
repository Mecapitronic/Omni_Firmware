...
bool Preferences::clear(){
    ...
    esp_err_t err = ESP_OK;// = nvs_erase_all(_handle);
    if(err){
...
bool Preferences::remove(const char * key){
    ...
    esp_err_t err = ESP_OK;// = nvs_erase_key(_handle, key);
    if(err){
...
size_t Preferences::putChar(const char* key, int8_t value){
    ...
    esp_err_t err = ESP_OK;// = nvs_set_i8(_handle, key, value);
    if(err){
...
size_t Preferences::putUChar(const char* key, uint8_t value){
    ...
    esp_err_t err = ESP_OK;// = nvs_set_u8(_handle, key, value);
    if(err){
...
size_t Preferences::putShort(const char* key, int16_t value){
    ...
    esp_err_t err = ESP_OK;// = nvs_set_i16(_handle, key, value);
    if(err){
...
size_t Preferences::putUShort(const char* key, uint16_t value){
    ...
    esp_err_t err = ESP_OK;// = nvs_set_u16(_handle, key, value);
    if(err){
...
size_t Preferences::putInt(const char* key, int32_t value){
    ...
    esp_err_t err = ESP_OK;// = nvs_set_i32(_handle, key, value);
    if(err){
...
size_t Preferences::putUInt(const char* key, uint32_t value){
    ...
    esp_err_t err = ESP_OK;// = nvs_set_u32(_handle, key, value);
    if(err){
...
size_t Preferences::putLong64(const char* key, int64_t value){
    ...
    esp_err_t err = ESP_OK;// = nvs_set_i64(_handle, key, value);
    if(err){
...
size_t Preferences::putULong64(const char* key, uint64_t value){
    ...
    esp_err_t err = ESP_OK;// = nvs_set_u64(_handle, key, value);
    if(err){
...
size_t Preferences::putString(const char* key, const char* value){
    ...
    esp_err_t err = ESP_OK;// = nvs_set_str(_handle, key, value);
    if(err){
...
size_t Preferences::putBytes(const char* key, const void* value, size_t len){
    ...
    esp_err_t err = ESP_OK;// = nvs_set_blob(_handle, key, value, len);
    if(err){
...
int8_t Preferences::getChar(const char* key, const int8_t defaultValue){
    ...
    esp_err_t err = ESP_OK;// = nvs_get_i8(_handle, key, &value);
    if(err){
...
uint8_t Preferences::getUChar(const char* key, const uint8_t defaultValue){
    ...
    esp_err_t err = ESP_OK;// = nvs_get_u8(_handle, key, &value);
    if(err){
...
int16_t Preferences::getShort(const char* key, const int16_t defaultValue){
    ...
    esp_err_t err = ESP_OK;// = nvs_get_i16(_handle, key, &value);
    if(err){
...
uint16_t Preferences::getUShort(const char* key, const uint16_t defaultValue){
    ...
    esp_err_t err = ESP_OK;// = nvs_get_u16(_handle, key, &value);
    if(err){
...
int32_t Preferences::getInt(const char* key, const int32_t defaultValue){
    ...
    esp_err_t err = ESP_OK;// = nvs_get_i32(_handle, key, &value);
    if(err){
...
uint32_t Preferences::getUInt(const char* key, const uint32_t defaultValue){
    ...
    esp_err_t err = ESP_OK;// = nvs_get_u32(_handle, key, &value);
    if(err){
...
int64_t Preferences::getLong64(const char* key, const int64_t defaultValue){
    ...
    esp_err_t err = ESP_OK;// = nvs_get_i64(_handle, key, &value);
    if(err){
...
uint64_t Preferences::getULong64(const char* key, const uint64_t defaultValue){
    ...
    esp_err_t err = ESP_OK;// = nvs_get_u64(_handle, key, &value);
    if(err){
...
size_t Preferences::getString(const char* key, char* value, const size_t maxLen){
    ...
    esp_err_t err = ESP_OK;// = nvs_get_str(_handle, key, NULL, &len);
    if(err){
...
String Preferences::getString(const char* key, const String defaultValue){
    ...
    esp_err_t err = ESP_OK;// = nvs_get_str(_handle, key, value, &len);
    if(err){
        //log_e("nvs_get_str len fail: %s %s", key, nvs_error(err));
        return String(defaultValue);
    }
...
size_t Preferences::getBytesLength(const char* key){
    ...
    esp_err_t err = ESP_OK;// = nvs_get_blob(_handle, key, NULL, &len);
    if(err){
...
size_t Preferences::getBytes(const char* key, void * buf, size_t maxLen){
    ...
    esp_err_t err = ESP_OK;// = nvs_get_blob(_handle, key, buf, &len);
    if(err){
...
size_t Preferences::freeEntries() {
    ...
    esp_err_t err = ESP_OK;// = nvs_get_stats(NULL, &nvs_stats);
    if(err){
