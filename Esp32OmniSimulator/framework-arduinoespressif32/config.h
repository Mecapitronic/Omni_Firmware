...
#define DXL_BUF_LENGTH                                                                   \
    (DXL_MAX_NODE * DXL_MAX_NODE_BUFFER_SIZE                                             \
     + 11) // 11 = Header(3)+Reserved(1)+ID(1)+Length(2)+Instruction(1)+Error(1)+crc(2)
...
#if defined(ARDUINO)
#include <Arduino.h>
#if !defined(ESP_PLATFORM) && !defined(ARDUINO_ARCH_MBED_PORTENTA)                       \
