...
/// @defgroup PixelTypes Pixel Data Types (CRGBFL/CHSV)
...
///
/// ![FastLED 'Rainbow' Hue
/// Chart](https://raw.githubusercontent.com/FastLED/FastLED/gh-pages/images/HSV-rainbow-with-desc.jpg)
///
/// @param hsv CHSV struct to convert to RGB. Max hue supported is HUE_MAX_RAINBOW
/// @param rgb CRGBFL struct to store the result of the conversion (will be modified)
void hsv2rgb_rainbow(const struct CHSV &hsv, struct CRGBFL &rgb);
...
struct CRGBFL
{
    ...
    static CRGBFL blend(const CRGBFL &p1, const CRGBFL &p2, fract8 amountOfP2);
    static CRGBFL blendAlphaMaxChannel(const CRGBFL &upper, const CRGBFL &lower);

    /// Array access operator to index into the CRGBFL object
    /// @param x the index to retrieve (0-2)
    /// @returns the CRGBFL::raw value for the given index
    ...
    /// @param ib input blue value
     CRGBFL(uint8_t ir, uint8_t ig, uint8_t ib) noexcept : r(ir), g(ig), b(ib) {}

    /// Allow construction from 32-bit (really 24-bit) bit 0xRRGGBB color code
    /// @param colorcode a packed 24 bit color code
     CRGBFL(uint32_t colorcode) noexcept
    ...
    /// Allow copy construction
     CRGBFL(const CRGBFL &rhs) = default;

    /// Allow construction from a CHSV color
     CRGBFL(const CHSV &rhs)
    {
        hsv2rgb_rainbow(rhs, *this);
    }

    /// Allow assignment from one RGB struct to another
     CRGBFL &operator=(const CRGBFL &rhs) = default;

    /// Allow assignment from 32-bit (really 24-bit) 0xRRGGBB color code
    /// @param colorcode a packed 24 bit color code
     CRGBFL &operator=(const uint32_t colorcode)
    {
        r = (colorcode >> 16) & 0xFF;
        g = (colorcode >> 8) & 0xFF;
        b = (colorcode >> 0) & 0xFF;
    ...
    /// @param nb new blue value
     CRGBFL &setRGB(uint8_t nr, uint8_t ng, uint8_t nb)
    {
        r = nr;
        g = ng;
        b = nb;
    ...
    /// @param val color value (brightness)
     CRGBFL &setHSV(uint8_t hue, uint8_t sat, uint8_t val)
    {
    ...
    /// Allow assignment from HSV color
     CRGBFL &operator=(const CHSV &rhs)
    {
        hsv2rgb_rainbow(rhs, *this);
        return *this;
    }
    ...
    /// Allow assignment from 32-bit (really 24-bit) 0xRRGGBB color code
    /// @param colorcode a packed 24 bit color code
     CRGBFL &setColorCode(uint32_t colorcode)
    {
        r = (colorcode >> 16) & 0xFF;
        g = (colorcode >> 8) & 0xFF;
        b = (colorcode >> 0) & 0xFF;
    ...
    /// Add one CRGBFL to another, saturating at 0xFF for each channel
     CRGBFL &operator+=(const CRGBFL &rhs);

    /// Add a constant to each channel, saturating at 0xFF.
    /// @note This is NOT an operator+= overload because the compiler
    /// can't usefully decide when it's being passed a 32-bit
    /// constant (e.g. CRGBFL::Red) and an 8-bit one (CRGBFL::Blue)
     CRGBFL &addToRGB(uint8_t d);

    /// Subtract one CRGBFL from another, saturating at 0x00 for each channel
     CRGBFL &operator-=(const CRGBFL &rhs);
    ...
    /// can't usefully decide when it's being passed a 32-bit
    /// constant (e.g. CRGBFL::Red) and an 8-bit one (CRGBFL::Blue)
     CRGBFL &subtractFromRGB(uint8_t d);
    ...
    /// @see nscale8x3_video
     CRGBFL &nscale8_video(uint8_t scaledown);

    /// %= is a synonym for nscale8_video().  Think of it is scaling down
    /// by "a percentage"
     CRGBFL &operator%=(uint8_t scaledown);
    ...
    /// fadefactor)
     CRGBFL &fadeLightBy(uint8_t fadefactor);
    ...
    /// @see nscale8x3
    CRGBFL &nscale8(uint8_t scaledown);

    /// Scale down a RGB to N/256ths of its current brightness, using
    /// "plain math" dimming rules. "Plain math" dimming rules means that the low light
    /// levels may dim all the way to 100% black.
    /// @see ::scale8
     CRGBFL &nscale8(const CRGBFL &scaledown);

     CRGBFL nscale8_(const CRGBFL scaledown) const;


    /// Return a CRGBFL object that is a scaled down version of this object
     CRGBFL scale8(uint8_t scaledown) const;

    /// Return a CRGBFL object that is a scaled down version of this object
     CRGBFL scale8(const CRGBFL &scaledown) const;

    /// fadeToBlackBy is a synonym for nscale8(), as a fade instead of a scale
    /// @param fadefactor the amount to fade, sent to nscale8() as (255 - fadefactor)
     CRGBFL &fadeToBlackBy(uint8_t fadefactor);
    ...
    /// This allows testing a CRGBFL for zero-ness
     explicit operator bool() const
    {
        return r || g || b;
    }

    /// Converts a CRGBFL to a 32-bit color having an alpha of 255.
    ...
    /// Invert each channel
     CRGBFL operator-() const
    {
        return CRGBFL(255 - r, 255 - g, 255 - b);
    }
    ...
    /// Get the "luma" of a CRGBFL object. In other words, roughly how much
    /// light the CRGBFL pixel is putting out (from 0 to 255).
    ...
    /// Maximize the brightness of this CRGBFL object.
    ...
    /// Calculates the combined color adjustment to the LEDs at a given scale, color
    /// correction, and color temperature
    ...
    /// @returns a CRGBFL object representing the adjustment, including color correction and
    /// color temperature
    static CRGBFL computeAdjustment(uint8_t scale,
                                  const CRGBFL &colorCorrection,
                                  const CRGBFL &colorTemperature);

    /// Return a new CRGBFL object after performing a linear interpolation between this
    /// object and the passed in object
     CRGBFL lerp8(const CRGBFL &other, fract8 frac) const;

    /// @copydoc lerp8
     CRGBFL lerp16(const CRGBFL &other, fract16 frac) const;
...
/// Check if two CRGBFL objects have the same color data
 bool operator==(const CRGBFL &lhs, const CRGBFL &rhs)
{
    return (lhs.r == rhs.r) && (lhs.g == rhs.g) && (lhs.b == rhs.b);
}

/// Check if two CRGBFL objects do *not* have the same color data
 bool operator!=(const CRGBFL &lhs, const CRGBFL &rhs)
{
    return !(lhs == rhs);
}
...
/// Check if the sum of the color channels in one CRGBFL object is less than another
 bool operator<(const CRGBFL &lhs, const CRGBFL &rhs)
{
    uint16_t sl, sr;
...
/// Check if the sum of the color channels in one CRGBFL object is greater than another
 bool operator>(const CRGBFL &lhs, const CRGBFL &rhs)
{
    uint16_t sl, sr;
    sl = lhs.r + lhs.g + lhs.b;
...
/// Check if the sum of the color channels in one CRGBFL object is greater than or equal to
/// another
 bool operator>=(const CRGBFL &lhs, const CRGBFL &rhs)
{
    uint16_t sl, sr;
...
/// Check if the sum of the color channels in one CRGBFL object is less than or equal to
/// another
 bool operator<=(const CRGBFL &lhs, const CRGBFL &rhs)
{
    uint16_t sl, sr;
...
/// @copydoc CRGBFL::operator/=
 CRGBFL operator/(const CRGBFL &p1, uint8_t d)
{
    return CRGBFL(p1.r / d, p1.g / d, p1.b / d);
}

/// Combine two CRGBFL objects, taking the smallest value of each channel
 CRGBFL operator&(const CRGBFL &p1, const CRGBFL &p2)
{
    return CRGBFL(
        p1.r < p2.r ? p1.r : p2.r, p1.g < p2.g ? p1.g : p2.g, p1.b < p2.b ? p1.b : p2.b);
}

/// Combine two CRGBFL objects, taking the largest value of each channel
 CRGBFL operator|(const CRGBFL &p1, const CRGBFL &p2)
{
    return CRGBFL(
        p1.r > p2.r ? p1.r : p2.r, p1.g > p2.g ? p1.g : p2.g, p1.b > p2.b ? p1.b : p2.b);
...
/// Scale using CRGBFL::nscale8_video()
 CRGBFL operator%(const CRGBFL &p1, uint8_t d);
