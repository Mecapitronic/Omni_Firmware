...
class CFastLED
{
    ...
    power_func
        m_pPowerFunc; ///< function for overriding brightness when using FastLED.show();

public:
    CFastLED();

    // Useful when you want to know when an event like onFrameBegin or onFrameEnd is
    // happening. This is disabled on AVR to save space.
    ...
    //{
    //    //fl::EngineEvents::removeListener(listener);
    //}

    /// Add a CLEDController instance to the world.  Exposed to the public to allow people
    /// to implement their own CLEDController objects or instances.  There are two ways to
    /// call this method (as well as the other addLeds() variations). The first is with 3
    /// arguments, in which case the arguments are the controller, a pointer to led data,
    /// and the number of leds used by this controller.  The second is with 4 arguments,
    /// in which case the first two arguments are the same, the third argument is an
    /// offset into the CRGB data where this controller's CRGB data begins, and the fourth
    /// argument is the number of leds for this controller object.
    /// @param pLed the led controller being added
    /// @param data base pointer to an array of CRGB data structures
    /// @param nLedsOrOffset number of leds (3 argument version) or offset into the data
    /// array
    /// @param nLedsIfOffset number of leds (4 argument version)
    /// @returns a reference to the added controller
    static CLEDController &addLeds(CLEDController *pLed,
                                   struct CRGB *data,
                                   int nLedsOrOffset,
                                   int nLedsIfOffset = 0);
    ...
    /// variations).  The first is with 2 arguments, in which case the arguments are  a
    /// pointer to led data, and the number of leds used by this controller.  The second
    /// is with 3 arguments, in which case the first  argument is the same, the second
    /// argument is an offset into the CRGB data where this controller's CRGB data begins,
    /// and the third argument is the number of leds for this controller object.
    ///
    /// @param data base pointer to an array of CRGB data structures
    /// @param nLedsOrOffset number of leds (3 argument version) or offset into the data
    /// array
    ...
    /// @tparam DATA_PIN the optional data pin for the leds (if omitted, will default to
    /// the first hardware SPI MOSI pin)
    /// @tparam CLOCK_PIN the optional clock pin for the leds (if omitted, will default to
    /// the first hardware SPI clock pin)
    /// @tparam RGB_ORDER the rgb ordering for the leds (e.g. what order red, green, and
    /// blue data is written out in)
    /// @tparam SPI_DATA_RATE the data rate to drive the SPI clock at, defined using
    /// DATA_RATE_MHZ or DATA_RATE_KHZ macros
    /// @returns a reference to the added controller
    /// @{
...
#define _FL_MAP_CLOCKED_CHIPSET(CHIPSET_ENUM, CONTROLLER_CLASS)                          \
    ...
    {                                                                                    \
        ...
        template <EOrder RGB_ORDER> struct CONTROLLER_CLASS_WITH_ORDER                   \
        {                                                                                \
        ...
        {                                                                                \
            typedef CONTROLLER_CLASS<DATA_PIN, CLOCK_PIN, RGB_ORDER, FREQ>               \
                ControllerType;                                                          \
        };                                                                               \
    };
    ...
    CLEDController &addLeds(struct CRGB *data, int nLedsOrOffset, int nLedsIfOffset = 0)
    {
        ...
        typedef typename CHIP::template CONTROLLER_CLASS_WITH_ORDER_AND_FREQ<
            RGB_ORDER,
            SPI_DATA_RATE>::ControllerType ControllerTypeWithFreq;
    ...
    /// Add an SPI based CLEDController instance to the world.
    template <ESPIChipsets CHIPSET, uint8_t DATA_PIN, uint8_t CLOCK_PIN>
    static CLEDController &addLeds(struct CRGB *data,
                                   int nLedsOrOffset,
                                   int nLedsIfOffset = 0)
    {
        typedef ClockedChipsetHelper<CHIPSET, DATA_PIN, CLOCK_PIN> CHIP;
    ...
    CLEDController &addLeds(struct CRGB *data, int nLedsOrOffset, int nLedsIfOffset = 0)
    {
        ...
        static_assert(CHIP::IS_VALID, "Unsupported chipset");
        typedef
            typename CHIP::template CONTROLLER_CLASS_WITH_ORDER<RGB_ORDER>::ControllerType
                ControllerTypeWithOrder;
    ...
    static CLEDController &addLeds(struct CRGB *data,
                                   ...
                                   int nLedsIfOffset = 0)
    {
        return addLeds<CHIPSET, SPI_DATA, SPI_CLOCK, RGB>(
    ...
    static CLEDController &addLeds(struct CRGB *data,
                                   ...
                                   int nLedsIfOffset = 0)
    {
        return addLeds<CHIPSET, SPI_DATA, SPI_CLOCK, RGB_ORDER>(
            data, nLedsOrOffset, nLedsIfOffset);
    }
    ...
    static CLEDController &addLeds(struct CRGB *data,
                                   ...
                                   int nLedsIfOffset = 0)
    {
        return addLeds<CHIPSET, SPI_DATA, SPI_CLOCK, RGB_ORDER, SPI_DATA_RATE>(
            data, nLedsOrOffset, nLedsIfOffset);
    }
    ...
    /// and the third argument is the number of leds for this controller object.
    ///
    /// This method also takes 2 to 3 template parameters for identifying the specific
    /// chipset, data pin, RGB ordering, and SPI data rate
    ///
    /// @param data base pointer to an array of CRGB data structures
    /// @param nLedsOrOffset number of leds (3 argument version) or offset into the data
    /// array
    ...
    template <template <uint8_t DATA_PIN, EOrder RGB_ORDER> class CHIPSET,
              uint8_t DATA_PIN,
              EOrder RGB_ORDER>
    static CLEDController &addLeds(struct CRGB *data,
                                   int nLedsOrOffset,
    ...
    /// is with 3 arguments, in which case the first  argument is the same, the second
    /// argument is an offset into the CRGB data where this controller's CRGB data begins,
    /// and the third argument is the number of leds for this controller object. This
    /// class includes the SmartMatrix and OctoWS2811 based controllers
    ///
    /// This method also takes 1 to 2 template parameters for identifying the specific
    /// chipset and RGB ordering.
    ///
    /// @param data base pointer to an array of CRGB data structures
    /// @param nLedsOrOffset number of leds (3 argument version) or offset into the data
    /// array
    ...
    /// and the third argument is the number of leds for this controller object.
    ///
    /// This method also takes a 2 to 3 template parameters for identifying the specific
    /// chipset and rgb ordering RGB ordering, and SPI data rate
    ///
    /// @param data base pointer to an array of CRGB data structures
    /// @param nLedsOrOffset number of leds (3 argument version) or offset into the data
    /// array
...
#ifdef PORTA_FIRST_PIN
        case WS2811_PORTA:
            return addLeds(new InlineBlockClocklessController<NUM_LANES,
                                                              PORTA_FIRST_PIN,
                                                              NS(320),
                                                              NS(320),
                                                              NS(640),
...
#ifdef HAS_PORTDC
        case WS2811_PORTDC:
            return addLeds(new SixteenWayInlineBlockClocklessController<NUM_LANES,
                                                                        NS(320),
                                                                        NS(320),
                                                                        NS(640),
                                                                        RGB_ORDER>(),
    ...
    /// Update all our controllers with the current led colors, using the passed in
    /// brightness
    ...
    /// Delay for the given number of milliseconds.  Provided to allow the library to be
    /// used on platforms that don't have a delay function (to allow code to be more
    /// portable).
    /// @note This will call show() constantly to drive the dithering engine (and will
    /// call show() at least once).
    /// @param ms the number of milliseconds to pause for
    void delay(unsigned long ms);

    /// Set a global color temperature.  Sets the color temperature for all added led
    /// strips, overriding whatever previous color temperature those controllers may have
    /// had.
    /// @param temp A CRGB structure describing the color temperature
    void setTemperature(const struct CRGB &temp);

    /// Set a global color correction.  Sets the color correction for all added led
    /// strips, overriding whatever previous color correction those controllers may have
    /// had.
    /// @param correction A CRGB structure describin the color correction.
    void setCorrection(const struct CRGB &correction);

    /// Set the dithering mode.  Sets the dithering mode for all added led strips,
    /// overriding whatever previous dithering option those controllers may have had.
    /// @param ditherMode what type of dithering to use, either BINARY_DITHER or
    /// DISABLE_DITHER

    /// Set the maximum refresh rate.  This is global for all leds.  Attempts to
    /// call show() faster than this rate will simply wait.
    /// @note The refresh rate defaults to the slowest refresh rate of all the leds added
    /// through addLeds(). If you wish to set/override this rate, be sure to call
    /// setMaxRefreshRate() _after_ adding all of your leds.
