...
// uint32_t CRGBFL::Squant = ((uint32_t)((__TIME__[4]-'0') * 28))<<16 |
...
//void CFastLED::showColor(const struct CRGBFL &color, uint8_t scale)
//{
//    //while (m_nMinMicros && ((micros() - lastshow) < m_nMinMicros))
//    //    ;
//    //lastshow = micros();
...
//void CFastLED::setTemperature(const struct CRGBFL &temp)
//{
//    CLEDController *pCur = CLEDController::head();
//    while (pCur)
//    {
...
//}
//
//void CFastLED::setCorrection(const struct CRGBFL &correction)
//{
//    CLEDController *pCur = CLEDController::head();
//    while (pCur)
//    {
...
void CFastLED::setMaxRefreshRate(uint16_t refresh, bool constrain)
{
    if (constrain)
    {
        // if we're constraining, the new value of m_nMinMicros _must_ be higher than
        // previously (because we're only allowed to slow things down if constraining)
        if (refresh > 0)
        {
            m_nMinMicros = ((1000000 / refresh) > m_nMinMicros) ? (1000000 / refresh)
                                                                : m_nMinMicros;
        }
    }
    else if (refresh > 0)
