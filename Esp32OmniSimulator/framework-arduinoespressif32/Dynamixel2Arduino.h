...
public:
     ...
     * @endcode
     * @param port The HardwareSerial port you want to use on the board to communicate
     * with DYNAMIXELs. It is automatically initialized baudrate to 57600 by calling the
     * begin () function.
     * @param dir_pin Directional pins for using half-duplex communication. -1 uses full
     * duplex. (default : -1)
     */
    Dynamixel2Arduino(HardwareSerial &port,
                      int dir_pin = -1,
                      uint16_t packet_buf_size = DEFAULT_DXL_BUF_LENGTH);
     ...
     * dxl.begin(57600);
     * @endcode
     * @param baud The port speed you want on the board (the speed to communicate with
     * DYNAMIXEL) (default : 57600)
     ...
     * dxl.scan();
     * @endcode
     * @return If a dynamixel succeeds in pinging, it returns 1, and returns 0 if none
     * succeeds.
     */
    bool scan();
     ...
     * @endcode
     * @param id DYNAMIXEL Actuator's ID.
     * @param baudrate DYNAMIXEL Actuator's baudrate value. (Refer to the control table of
     * each actuator for supported values (e-manual))
     * @return It returns true(1) on success, false(0) on failure.
     */
     ...
     * @endcode
     * @param id DYNAMIXEL Actuator's ID.
     * @param unit The unit you want to return (the function converts the raw value to the
     * unit you specified and returns it) (default : UNIT_RAW) Only support UNIT_RAW,
     * UNIT_DEGREE.
     * @return It returns the data read from DXL control table item.(Returns the value
     * appropriate for @unit.) If the read fails, 0 is returned. Whether or not this is an
     * actual value can be confirmed with @getLastLibErrCode().
     */
     ...
     * @param unit The unit you want to return (the function converts the raw value to the
     * unit you specified and returns it) (default : UNIT_RAW) Only support UNIT_RAW,
     * UNIT_PERCENT, UNIT_MILLI_AMPERE.
     * @return It returns the data read from DXL control table item.(Returns the value
     * appropriate for @unit.) If the read fails, 0 is returned. Whether or not this is an
     * actual value can be confirmed with @getLastLibErrCode().
     */
     ...
     *    For each index, replace the name of the control table item in the e-manual with
     * capital letters and replace the space with an underscore (_). For the list of
     * indexes, please refer to the link below.
     ...
     *    TODO: add link (It is defined as 'enum ControlTableItem' in actuator.h)
     * @param id DYNAMIXEL Actuator's ID.
     * @param data The data you want to write. Only the data size allowed by the address
     * is applied.
     * @param timeout A timeout waiting for a response to a data transfer.
     * @return It returns true(1) on success, false(0) on failure.
     */
...
private:
    ...
    //bool writeForRangeDependencyFunc(uint8_t func_idx,
    //                                 uint8_t id,
    //                                 float value,
    //                                 uint8_t unit);
