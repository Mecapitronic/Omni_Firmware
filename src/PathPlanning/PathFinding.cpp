...
namespace PathFinding
{
    ...
    boolean PathFinding(int16_t start_x, int16_t start_y, t_vertexID end_vertex_ID)
    {
        ...
        while (open.size() > 0)
        {
            ...
            // on regarde si le noeud est le noeud final OU si il y a condition d'arrét
            // anticipé
            if ((Is_Equal_Vertex(best.currentID,
                                 Get_End_Vertex()))) //|| (PATH_FINDING_STOP))
            {
                int i = 0;
                int pos = 0;
                ...
                {
                    auto it = std::find_if(
                        close.begin(), close.end(), [&dummy](const NodeItem &node) {
    ...
    void PathFindingAddToOpen(std::vector<NodeItem> &list)
    {
        ...
        for (size_t i = 0; i < list.size(); i++)
        {
            ...
            auto itOpen =
                std::find_if(open.begin(), open.end(), [&dummy](const NodeItem &node) {
                    return Mapping::Is_Equal_Vertex(node.currentID, dummy.currentID);
                });
            ...
            if (itOpen == open.end())
            {
                ...
                if (itClose == close.end())
                {
                    ...
                    // dummyOpen = list[i];
                    insert_sorted(
                        open, dummy, [](const NodeItem &node1, const NodeItem &node2) {
                            return node2.FCmp(node1) > 0;
                        });
                }
                else
                {
            ...
            else
            {
                ...
                {
                    // we need to remove it from open then put it back sorted because it
                    // may not be at the right place
                    open.erase(itOpen);
    ...
    void ListPrint(std::vector<NodeItem> &list, String str)
    {
        ...
        for (i = 0; i < list.size(); i++)
        {
            ...
            // printf("Heuristic %f , Cost %f " , NodeF(current->data) ,
            // NodeGetCost(current->data));
            printf("\n");
        }
    }
