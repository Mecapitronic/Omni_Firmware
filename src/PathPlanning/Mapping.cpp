...
namespace Mapping
{
    ...
    void Initialize_Map(Team team)
    {
        ...
        {
            // Segment
            segment[0] = Segment(Point(650, 1800), Point(1050, 1800));
            segment[1] = Segment(Point(1050, 1800), Point(1050, 1550));
            segment[2] = Segment(Point(1950, 1550), Point(1950, 1800));
            segment[3] = Segment(Point(1950, 1800), Point(2350, 1800));
            // Circle
            circle[0] = Circle(950, 950, 50);
            circle[1] = Circle(1050, 950, 50);
            circle[2] = Circle(1150, 950, 50);
            circle[3] = Circle(1250, 950, 50);
            circle[4] = Circle(1750, 950, 50);
            circle[5] = Circle(1850, 950, 50);
            circle[6] = Circle(1950, 950, 50);
            circle[7] = Circle(2050, 950, 50);
            circle[8] = Circle(625, 250, 50);
            circle[9] = Circle(925, 250, 50);
            ...
            vertex[6].point = Point(1100, 700);
            vertex[7].point = Point(1100, 1150);
            ...
            vertex[15].point = Point(2225, 450);
            vertex[16].point = Point(2225, 175);
            vertex[17].point = Point(2750, 400);
            vertex[18].point = Point(2400, 875);
            vertex[19].point = Point(1900, 700);
            vertex[20].point = Point(1900, 1150);
        ...
        {
            // Segment
            segment[0] = Segment(Point(2350, 1800), Point(1950, 1800));
            segment[1] = Segment(Point(1950, 1800), Point(1950, 1550));
            segment[2] = Segment(Point(1050, 1550), Point(1050, 1800));
            segment[3] = Segment(Point(1050, 1800), Point(650, 1800));
            // Circle
            circle[0] = Circle(2050, 950, 50);
            circle[1] = Circle(1950, 950, 50);
            circle[2] = Circle(1850, 950, 50);
            circle[3] = Circle(1750, 950, 50);
            circle[4] = Circle(1250, 950, 50);
            circle[5] = Circle(1150, 950, 50);
            circle[6] = Circle(1050, 950, 50);
            circle[7] = Circle(950, 950, 50);
            circle[8] = Circle(2375, 250, 50);
            circle[9] = Circle(2075, 250, 50);
            ...
            vertex[6].point = Point(1900, 700);
            vertex[7].point = Point(1900, 1150);
            ...
            vertex[15].point = Point(775, 450);
            vertex[16].point = Point(775, 175);
            vertex[17].point = Point(250, 400);
            vertex[18].point = Point(600, 875);
            vertex[19].point = Point(1100, 700);
            vertex[20].point = Point(1100, 1150);
    ...
    boolean Is_Intersection_Segment(Segment *s1, Segment *s2)
    {
        ...
        if ((s1->a == s2->a)
            && (s1->b
                == s2->b)) // segments are one the same line (parallel but not distinct)
    ...
    /****************************************************************************************
     * Return 1 if point p is between end points of segment s (/!\ it's assume that p is
     *on the line)
     ****************************************************************************************/
    ...
    boolean Is_Circle_CloseTo_Segment(Circle *c, Segment *s, uint16_t d)
    {
        ...
        if (Get_Distance_Point(&s->p1, &c->p) <= d)
            return true;
        if (Get_Distance_Point(&s->p2, &c->p) <= d)
            return true;
    ...
    boolean Is_Segment_CloseTo_Segment(Segment *s1, Segment *s2, uint16_t d)
    {
        ...
        if (Is_Intersection_Segment(s1, s2))
            return true;

        //  // check distance from each end point to each other       // manually done in
        //  excel map if (Get_Distance_Point(&s1->p1, &s2->p1) <= d) return true; if
        //  (Get_Distance_Point(&s1->p1, &s2->p2) <= d) return true; if
        //  (Get_Distance_Point(&s1->p2, &s2->p1) <= d) return true; if
        //  (Get_Distance_Point(&s1->p2, &s2->p2) <= d) return true;

        //  // check distance orthogonal from end point               // manually done in
        //  excel map if (Is_Point_CloseTo_Segment(&s1->p1, s2, d)) return true; if
        //  (Is_Point_CloseTo_Segment(&s1->p2, s2, d)) return true; if
        //  (Is_Point_CloseTo_Segment(&s2->p1, s1, d)) return true; if
        //  (Is_Point_CloseTo_Segment(&s2->p2, s1, d)) return true;
    ...
    /****************************************************************************************
     * Return 1 if point target is passable from point source, according to distance
     *margin
     ****************************************************************************************/
    boolean Is_Passable_Point(Point *source, Point *target, uint16_t margin)
    {
        ...
        {
            if (Is_Segment_CloseTo_Segment(
                    &segment[i], &edge, margin)) // check segment proximity
                return false;
        }
        ...
        for (i = 0; i < Max_Circle; i++)
        {
            ...
            {
                if (Is_Circle_CloseTo_Segment(
                        &circle[i], &edge, margin)) // check circle proximity
                    return false;
            }
        }
    ...
    /****************************************************************************************
     * Return 1 if point target is passable from robot (vertex[0]), according to distance
     *margin
     ****************************************************************************************/
    boolean Is_Passable_Robot(Point *target, uint16_t margin)
    {
        ...
        {
            // check segment intersection
            if (Is_Intersection_Segment(&segment[i], &edge))
                return false;

            //    // check distance from each end point to each other
            //    //if (Get_Distance_Point(&edge.p1, &segment[i].p1) <= margin) return
            //    false; // ignore edge.p1 = vertex[0] = robot
            //    //if (Get_Distance_Point(&edge.p1, &segment[i].p2) <= margin) return
            //    false; if (Get_Distance_Point(&edge.p2, &segment[i].p1) <= margin)
            //    return false; if (Get_Distance_Point(&edge.p2, &segment[i].p2) <=
            //    margin) return false;

            //    // check distance orthogonal from end point
            //    if (Is_Point_CloseTo_Segment(&segment[i].p1, &edge, margin)) return
            //    false; if (Is_Point_CloseTo_Segment(&segment[i].p2, &edge, margin))
            //    return false;
            //    //if (Is_Point_CloseTo_Segment(edge.p1, segment[i], margin)) return
            //    false; if (Is_Point_CloseTo_Segment(&edge.p2, &segment[i], margin))
            //    return false;  // manually done in excel map
        }
        ...
        for (i = 0; i < Max_Circle; i++) // check circle proximity
        {
            if (Is_NotNull_Circle(&circle[i]))
            {
                ...
                // if (Get_Distance_Point(&edge.p1, &circle[i].p) <= margin_temp) return
                // false;
                if (Get_Distance_Point(&edge.p2, &circle[i].p) <= margin_temp)
                    return false; // usefull for vertex in circle

                if (Is_Point_CloseTo_Segment(&circle[i].p, &edge, margin_temp))
                    return false; // most important
            }
        }
    ...
    void Initialize_Passability_Graph(void)
    {
        ...
        {
            for (j = (i + 1); j < Max_Vertex;
                 j++) // ignore vertex couple already checked (i,j)=(j,i)
            {
    ...
    void Update_Passability_Obstacle(void)
    {
        ...
        for (i = 1; i < Max_Vertex; i++)
        {
            vertex[i].adjacency_active =
                vertex[i].adjacency_passive; // retrieve list without obstacle
        }
    ...
    //   for (i=1; i<MAX_VERTEX; i++)
    //   {
    //     vertex[i].adjacency_active = vertex[i].adjacency_static;  // retrieve list
    //     without element
    //   }
    //
    ...
    //       for (i=1; i<MAX_VERTEX; i++)
    //       {
    //         for (j=(i+1); j<MAX_VERTEX; j++) // ignore vertex couple already checked
    //         (i,j)=(j,i)
    //         {
    //           if (Is_Adjacent(i, j))  // for all couple passabled
    //           {
    //             edge = Segment(vertex[i].point, vertex[j].point);
    //
    //             if (Is_Circle_CloseTo_Segment(&element[k], &edge, margin)) // check
    //             circle proximity
    //             {
    //               Clear_Adjacent(i,j);  // Temporarily not passabled
    //             }
    ...
    void PrintVertexList()
    {
        ...
        for (size_t i = 1; i < Max_Vertex; i++)
        {
            ...
            {
                String stri1 =
                    String(vertex[i].point.x) + ":" + String(vertex[i].point.y);
    ...
    void PrintSegmentList()
    {
        ...
        {
            if (segment[i].p1.x != 0 || segment[i].p1.y != 0 || segment[i].p2.x != 0
                || segment[i].p2.y != 0)
            {
                String stri1 = String(segment[i].p1.x) + ":" + String(segment[i].p1.y);
                String stri2 =
                    ":" + String(segment[i].p2.x) + ":" + String(segment[i].p2.y);
                str = str + ";" + stri1 + stri2;
            }
        }
        // str = str + "|xy";
    ...
    //         printf("vertex[%d].adjacency_static = %ld",i ,vertex[i].adjacency_static);
    //         printf(";\n"); printf("vertex[%d].point.x = %d",i ,vertex[i].point.x);
    //         printf(";\n"); printf("vertex[%d].point.y = %d",i ,vertex[i].point.y);
    //         printf(";\n");
    //     }
    // }
